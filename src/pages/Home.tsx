...
import { Palette, Users, Building, Phone, Mail, MapPin, Facebook, Instagram, Linkedin, X, Menu, CheckCircle, Send, ShoppingCart } from 'lucide-react';
...
function Home() {
  ...
  const portfolioData = [
    ...
    { src: scrubUniformeImage, alt: 'Scrub Hospitalar Verde com Símbolo Murela Brands (Imagem Final)', caption: 'Uniformes Hospitalares e Scrubs' },
  ];
      ...
      <header className={`navbar fixed w-full z-50 transition-all duration-300 ${scrolled ? 'navbar-scrolled' : ''}`}>
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-2">
            ...
            <Link to="/loja" className="nav-link capitalize">
        ...
        <div className={`mobile-menu ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
           {[
            ...
            <Link to="/loja" className="mobile-menu-link" onClick={() => setMobileMenuOpen(false)}>
              Loja Virtual
            </Link>
        </div>
      </header>
